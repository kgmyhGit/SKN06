...
select emp_name, upper(emp_name) as "대문자이름", lower(emp_name), char_length(emp_name)
from   emp
...
from   emp where  char_length(emp_name) >= 10;

select concat('$',salary,'원') from emp;
select concat(emp_name, '-', dept_name) from emp; # 같은 행의 값들을 합친다.
...
************************************************************************** */
select trim('       aaa          ');
select round(3.12345678); -- 자릿수 생략. 기본값: 0 -> 정수
select round(3.12345678, 2); -- 소숫점 2자리 이하에서 반올림. (양수: 소숫점 이하)
select round(398.765, -1);  -- 음수: 정수부
/*
-2 -1  0   1 2 3     자릿수 지정 - 그 이하에서 반올림
 3  9  8 . 7 6 5
*/
-- EMP 테이블에서 각 직원에 대해 직원ID(emp_id), 이름(emp_name), 급여(salary) 
...
- 날짜관련 함수
 ...
 adddate(hire_date, interval 3 day)
...
select adddate(curdate(), interval 3 month); -- 오늘 부터 3개월 이후 날짜?
select adddate(curdate(), interval -3 month);
select subdate(curdate(), interval 3 month);

select adddate(curdate(), interval 3 year);
select adddate(curdate(), interval 3 quarter); -- 3분기 이후(9개월 이후)
select adddate(curdate(), interval 43 day);

-- 날짜 차이
select datediff(curdate(), '2020-10-07'); # 뺀 날짜 수


-- EMP 테이블에서 부서이름(dept_name)이 'IT'인 직원들의 
-- '입사일(hire_date)로 부터 10일전', 입사일, '입사일로 부터 10일 후' 의 날짜를 조회. 
select  subdate(hire_date, interval 10 day) as "입사 10일전",
        hire_date as "입사일",
        adddate(hire_date, interval 10 day) as "입사 10일후"
from    emp
where   dept_name = 'IT';
...
from   emp
where  emp_id = 200;
-- %Y: 년, %m: 월, %d: 일, %H: 시, %i: 분, %s: 초, %W 요일, %p: 오전/오후

-- 각 직원의 이름(emp_name), 
-- 근무 개월수 (입사일에서 현재까지의 달 수)를 계산하여 조회. 
-- 근무개월수 내림차순으로 정렬.
select  emp_name,
		-- hire_date,
		timestampdiff(month, hire_date, curdate()) as "근무개월",
        timestampdiff(year, hire_date, curdate()) as "근무년수"
from    emp
order by 2 desc;

-- 250개월 이상 근무한 직원 정보를 조회
select * from emp
...
************************************************************************************* */
select null;
select ifnull(null, '값없음');
select ifnull("홍길동", '값없음');

-- EMP 테이블에서 직원의 ID(emp_id), 이름(emp_name), 업무(job),
--  부서(dept_name)을 조회. 부서가 없는 경우 '부서미배치'를 출력.
select  emp_id,
		emp_name,
        job,
        ifnull(dept_name, '부서미배치') as "dept_name"
from    emp
order by 4; -- 정렬시 select 절에서 처리된 값 기준으로 정렬.
...
select  emp_id,
		emp_name,
		format(salary, 2) as "salary", # 3단위당 , 출력. 2 - 소숫점 2자리까지만 나오게.
        comm_pct,
...
***********************************************/
-- 시간을 정수형태로 변환   
select cast(curtime() as signed); -- 144039  14:40:39
select cast(cast(curtime() as signed) as char); -- 144039 -> '144039'


-- 숫자를 날짜로 변환
select convert(20241007, date);

-- 숫자를 문자열로 변환
select convert(200, char);

select '10' + '20'; # +: 숫자에만 사용가능. 암묵적으로 값을 숫자로 변환후 계산.
select concat('10', 20);
...
select  salary,
		case when salary >= 10000 then '1등급' 
             when salary < 10000 then '2등급' end
from emp;
select  salary,
		case when salary >= 10000 then '1등급' 
...
select  salary,
		case when salary >= 10000 then '1등급' end as "급여등급"
from emp;

select salary, if(salary >= 10000, '1등급', '2등급') as "급여등급" from emp;


-- EMP 테이블에서 업무(job)이 'AD_PRES'거나 'FI_ACCOUNT'거나 
-- 'PU_CLERK'인 직원들의 ID(emp_id), 이름(emp_name), 업무(job)을 조회.  
-- 업무(job)가 'AD_PRES'는 '대표', 'FI_ACCOUNT'는 '회계', 
-- 'PU_CLERK'의 경우 '구매'가 출력되도록 조회
select emp_id, emp_name, 
	   job,
	   case job when 'AD_PRES' then '대표' 
                when 'FI_ACCOUNT' then '회계'
                when 'PU_CLERK' then '구매' 
                else job end as "job2" -- 반환값에 컬럼명 지정 -> 원래값 반환.
from emp
...
-- 급여 인상분은 부서이름이 'IT' 이면 급여(salary)에 10%를 
-- 'Shipping' 이면 급여(salary)의 20%를 
-- 'Finance'이면 30%를 나머지는 0을 출력
select  dept_name,
...
-- EMP 테이블에서 직원의 ID(emp_id), 이름(emp_name), 급여(salary), 
-- 인상된 급여를 조회한다. 
-- 단 급여 인상율은 급여가 5000 미만은 30%, 5000이상 
-- 10000 미만는 20% 10000 이상은 10% 로 한다.
select  emp_id,
		emp_name,
		salary,
        case when salary < 5000 then salary * 0.3
             when salary between 5000 and 9999.99 then salary * 0.2
             else salary * 0.1 end as "급여인상액"
from emp;