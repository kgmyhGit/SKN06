...
--  하는 직원의 id(emp_id),이름(emp.emp_name), 업무(emp.job_id),
--  급여(emp.salary) 조회
...
-- 직원_id(emp.emp_id)가 115번인 직원과 같은 업무(emp.job_id)를
--  하고 같은 부서(emp.dept_id)에 속한 직원들을 조회하시오.
select *
from   emp
...
-- 부서직원들의 평균급여가 전체 직원의 평균(emp.salary) 이상인 
-- 부서의 이름(dept.dept_name), 평균 급여(emp.salary) 조회.
-- 평균급여는 소숫점 2자리까지 나오고 통화표시($)와 단위 구분자 출력
select d.dept_name, 
	   concat('$', round(avg(e.salary),2)) as "평균급여" -- 문자열
...
select dept_name,  concat('$', 평균급여) as "평균급여"
...
-- from (subquery) -> subquery 실행 table로 부터 조회
--    inline view

--  급여(emp.salary)가장 많이 받는 직원이 속한 부서의 이름(dept.dept_name),
--   위치(dept.loc)를 조회.
select distinct d.dept_name, d.loc
from   dept d join emp e on d.dept_id = e.dept_id
...
-- 전체 직원들 중 담당 업무 ID(emp.job_id) 가 
-- 'ST_CLERK'인 직원들의 평균 급여보다 적은 급여를 받는 직원들의 모든 정보를 조회. 
...
-- 업무(emp.job_id)가 'IT_PROG' 인 직원들 중 가장 많은 급여를 받는 
-- 직원보다 더 많은 급여를 받는 직원들의 id(emp.emp_id), 
-- 이름(emp.emp_name), 급여(emp.salary)를 급여 내림차순으로 조회.
select emp_id, emp_name, salary
from   emp
where  salary > (select max(salary) from emp
                 where job_id = 'IT_PROG')
order by emp_name desc, salary desc;
...
--  부서 위치(dept.loc) 가 'New York'인 부서에 
-- 소속된 직원의 ID(emp.emp_id), 이름(emp.emp_name), 
...
-- 직원 ID(emp.emp_id)가 101, 102, 103 인 직원들 보다 급여(emp.salary)
-- 를 많이 받는 직원의 모든 정보를 조회.
select * from emp
where salary >all  (select salary from emp
			       where emp_id in (101, 102, 103));

-- 직원 ID(emp.emp_id)가 101, 102, 103 인 직원들 중 급여가 
-- 가장 적은 직원보다 급여를 많이 받는 직원의 모든 정보를 조회.
select * from emp
...
-- 전체 직원들중 부서_ID(emp.dept_id)가 20인 부서의 모든 직원들 보다 
-- 급여(emp.salary)를 많이 받는 직원들의 정보를 sub query를 이용해 조회.
select * from emp
...
-- 직원들의 id(emp.emp_id), 이름(emp.emp_name), 연봉(emp.salary), 
-- 소속부서ID(dept.dept_id) 조회
select *
from   emp e1
where  salary = (select max(salary) 
			     from emp 
                 where dept_id = e1.dept_id);
-- e1 테이블의 행을 where에서 조회할 때 마다 subquery를 실행.
-- subquery의 e1.dept_id는 e1의 where절에서 체크한 
--                                         행의 dept_id값을 의미한다.
...
************************************************************************************* */
-- 자식테이블이 부모테이블을 참조
-- 부모테이블의 행이 자식테이블에서 하나라도 참조되고 있는지.

-- 직원(자식)이 한명이상 있는 부서(부모)의 부서ID(dept.dept_id)와 
--    이름(dept.dept_name), 위치(dept.loc)를 조회
