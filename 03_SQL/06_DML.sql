...
-- 부서 ID가 100인 직원의 커미션 비율을 0.2로 
-- salary는 3000을 더한 값으로, 상사_id는 100 변경.
update emp
set    comm_pct = 0.2,
	   salary = salary + 3000,
       mgr_id = 100
...
-- EMP 테이블의 모든 데이터를 MGR_ID는 NULL로 
-- HIRE_DATE 는 현재일시로 COMM_PCT는 0.5로 수정.
use hr_join;
update emp
set    mgr_id = null,
	   hire_date = curdate(),
...
-- 담당 업무(emp.job_id)가 'SA_MAN'이고 급여(emp.salary) 가 
-- 12000 미만인 직원들을 삭제.
